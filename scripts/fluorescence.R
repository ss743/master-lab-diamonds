...
daten=read.csv(sprintf("../data/osa/012-001.csv"),sep=",",skip=33,header=FALSE,col.names=c("lambda","I"),dec=".")
l=length(daten[[1]])
...
range1=c(630,650);
fit1=posgausfit(fluorescence,range1,sig0=1.5);
pos=append(pos,fit1["mu","Estimate"])
spos=append(spos,fit1["mu","Std. Error"])
sigpos=append(sigpos,fit1["sig","Estimate"]/2)


ggplot(fluorescence,aes(x=x,y=y)) + geom_line(colour="black") +  xlab(expression(lambda/nm)) + ylab(expression(I)) + plotgausline(fit1,0.5) + plotgaus(fit1,range1)
